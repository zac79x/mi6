...
logged to `agentFour/agent.http.log` and `agentFour/agent.llm_payload.log`
...
agentFour implements the **Agent2Agent (A2A) Protocol v1.0** — an open
...
- **Role C (Client + Server combined):** agentFour can act as both an
...
from agentFour.a2a_server import create_a2a_server

agent = agentFour(model="your-model")
...
```python
from agentFour.a2a_client import A2AClient
...
agentFour/
  ...
  __main__.py         - `python -m agentFour` entry point
  ...
  agent.py            - The agentFour class: core loop, Ollama HTTP, streaming, compaction, cache
