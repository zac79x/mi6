...
from agentFour.agent import agentFour, DEFAULT_SESSION_FILE
from agentFour.cli_ui import (
...
from agentFour.config import DEFAULT_MODEL, DIFF_TOOL_PATH, LOG_FILE, OLLAMA_URL, available_models
from agentFour.logging_setup import logger
from agentFour.tools_registry import _TOOL_REGISTRY
...
def _local_files() -> list[str]:
    try:
        return sorted(f for f in os.listdir(".") if os.path.isfile(f) and (f.endswith(".py") or f.endswith(".json") or f.endswith(".md")))
    except OSError:
        return []


def _handle_command(cmd: str, agent: agentFour) -> bool:
    parts = cmd.split()
    head = parts[0].lower()
...
def main() -> None:
    ...
    agent = agentFour(model=DEFAULT_MODEL)
    completer = AgentCompleter(extra_files=_local_files(), workspace_root=str(WORKSPACE_ROOT))
