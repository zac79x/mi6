"""A2A Server: exposes agentThree as an A2A-compliant HTTP+JSON endpoint.
...
The server wraps an ``agentThree`` instance.  Each ``SendMessage`` creates
...
collect them via the A2A protocol without importing agentThree.
"""
...
from agentThree.a2a_models import (
...
def _collect_agent_metrics(agent) -> dict[str, Any]:
    """Extract benchmark metrics from an agentThree instance after chat()."""
...
def create_a2a_server(
    ...
    host: str = "localhost",
    port: int = 8080,
    agent_name: str = "agentThree",
    agent_description: str = "An Ollama-powered tool-calling agent exposed via A2A",
    base_url: str | None = None,
    skills: list[dict] | None = None,
) -> ThreadingHTTPServer:
    """Create an A2A HTTP server wrapping an ``agentThree`` instance.
